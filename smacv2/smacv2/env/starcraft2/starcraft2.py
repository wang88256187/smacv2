...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        if self.use_stacked_frames:
            self.stacked_local_obs = np.zeros(
                (self.n_agents, self.stacked_frames, int(self.get_obs_size()[0] / self.stacked_frames)),
                dtype=np.float32)
            self.stacked_global_state = np.zeros(
                (self.n_agents, self.stacked_frames, int(self.get_state_size()[0] / self.stacked_frames)),
                dtype=np.float32)
    ...
    def get_state_size(self):
        ...
        if self.use_state_agent:
            ...
            return [all_feats * self.stacked_frames if self.use_stacked_frames else all_feats, [n_allies, n_ally_feats],
                    [n_enemies, n_enemy_feats], [1, move_feats], [1, own_feats + agent_id_feats + timestep_feats]]
        ...
        return [size * self.stacked_frames if self.use_stacked_frames else size, [self.n_agents, nf_al],
                [self.n_enemies, nf_en], [1, move_state + obs_agent_size + timestep_state + agent_id_feats]]
